...
def create_config_file(temp_dir, input_file, input_lang, metadata=None):
    """Create config.txt file for the pipeline"""
    try:
        config_file = os.path.join(temp_dir, "config.txt")

        config_content = f"""# Beautification Configuration
...
def main():
    ...
    try:
        ...
        if os.path.exists(input_html_path):
            ...
            create_config_file(temp_dir, input_file, args.ilang, metadata)
