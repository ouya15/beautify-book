#!/usr/bin/env python3
"""
merge_and_build.py - Merge beautified pages and build final outputs
Combines: merge -> HTML -> TOC -> DOCX/EPUB/PDF

Usage: merge_and_build.py --temp-dir <path> [--title <title>] [--author <author>]
"""
...
def self_detect_lang(source_lang):
    """Detect language code from source language metadata."""
    if not source_lang:
        return 'en'
    ...
    return lang_map.get(source_lang_lower, 'en')
...
def main():
    parser = argparse.ArgumentParser(description='Merge beautified pages and build final outputs')
    parser.add_argument('--temp-dir', required=True, help='Temp directory path')
    parser.add_argument('--title', default=None, help='Book title (override config)')
    parser.add_argument('--author', default=None, help='Author name (override config)')
    parser.add_argument('--lang', default='auto', help='Language code (default: auto-detect from source)')
    ...
    # Use auto-detect or get from source language
    if args.lang == 'auto':
        source_lang = config.get('source_language', '')
        lang_code = self_detect_lang(source_lang) if source_lang else 'en'
    else:
        lang_code = args.lang
    
    lang_cfg = get_lang_config(lang_code)

    title = args.title or config.get('original_title', 'Beautified Book')
